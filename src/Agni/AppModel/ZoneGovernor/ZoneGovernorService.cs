  ...
  public sealed class ZoneGovernorService : Service
  {
    ...
    #region Public
      ...
      public int SendLog(string host, string appName, NFX.Log.Message[] data)
      {
        ...
        return (int)(Log.AgniZoneDestination.MAX_BUF_SIZE * m_CPULoadFactor);
      }
      ...
      public void RegisterSubordinateHost(Contracts.HostInfo host, Contracts.DynamicHostID? hid)
      {
        ...
        var zHosts = AgniSystem.HostMetabaseSection.ParentZone.ZoneGovernorHosts.Where(hh => !AgniSystem.HostName.IsSameRegionPath(hh.RegionPath));
      ...
      {
        var zone = AgniSystem.HostMetabaseSection.ParentZone;
        if (!Running || m_DynamicHostSlots == null) return new Contracts.DynamicHostID(null, zone.RegionPath);

        // TODO: Check zone
        var host = AgniSystem.Metabase.CatalogReg.NavigateHost(hostPath) as Metabank.SectionHost;
        if (!host.Dynamic) throw new AZGOVException("TODO: Host '0' is not dynamic".Args(hostPath));

        if (id == null) id = AgniSystem.GDIDProvider.GenerateOneGDID(SysConsts.GDID_NS_DYNAMIC_HOST, SysConsts.GDID_NAME_DYNAMIC_HOST).ToString();
        ...
        if (dhi == null)
        {
          ...
          dhi.Stamp = stamp;
          dhi.Owner = AgniSystem.HostName;
          dhi.Votes = 1;
          m_DynamicHostSlots.Register(dhi);

          var hosts = zone.ZoneGovernorHosts.Where(hh => !AgniSystem.HostName.IsSameRegionPath(hh.RegionPath));
    ...
    #region .pvt .impl
        ...
        private void registerSubordinateHost(Contracts.HostInfo host, Contracts.DynamicHostID? hid)
        {
          try
          {
            var shost = AgniSystem.Metabase.CatalogReg.NavigateHost(host.Name);
            if (!shost.HasDirectOrIndirectParentZoneGovernor(AgniSystem.HostMetabaseSection, iAmZoneGovernor: false, transcendNOC: false))
              throw new AZGOVException(StringConsts.AZGOV_REGISTER_SUBORDINATE_HOST_PARENT_ERROR.Args(AgniSystem.HostName, host.Name));
          }
          catch (Exception error)
          {
