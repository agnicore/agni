...
namespace Agni.AppModel.Terminal.Cmdlets
{
    public class Glue : Cmdlet
    {
        ...
        public override string Execute()
        {
            ...
            if (attr.Exists) glue.ClientLogLevel = attr.ValueAsEnum<NFX.Log.MessageType>(glue.ClientLogLevel);

            attr = m_Args.AttrByName("server-log-level");
            if (attr.Exists) glue.ServerLogLevel = attr.ValueAsEnum<NFX.Log.MessageType>(glue.ServerLogLevel);
