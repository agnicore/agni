...
namespace Agni.WebMessaging.Server
{
  /// <summary>
  ...
  {
    public AgniWebMessage FetchMailboxMessage(MailboxMsgID mid)
    {
    ...
    public MsgSendInfo[] SendMessage(AgniWebMessage msg)
    {
  ...
  public sealed class WebMessageSystemService : ServiceWithInstrumentationBase<object>, Contracts.IWebMessageSystem
  {
    ...
    public MsgSendInfo[] SendMessage(AgniWebMessage msg)
    {
      ...
      foreach (var adr in msg.AddressToBuilder.All)
      {
        ...
        {
          log(NFX.Log.MessageType.Critical, "sndmsg.chn.wrt", "Channel '{0}' leaked:".Args(channel.Name, error.ToMessageWithType()), error);
