...
namespace Agni.Metabase{ public sealed partial class Metabank{
  ...
  public sealed class SectionHost : SectionRegionBase
  {
      ...
      /// <summary>
      /// Returns a name of the role (as defined by the attribute in host confog file) that this host has in the agni
      /// </summary>
      public string RoleName { get{ return this.LevelConfig.AttrByName(CONFIG_ROLE_ATTR).ValueAsString(SysConsts.NULL); } }

      /// <summary>
      /// Returns a role app catalog section for the role that this host has in the agni
      /// </summary>
      ...
      /// The Dynamic host section represents a proptotype of (possibly) many host instances spawn in the agni.
         ...
         foreach(var app in Role.AppNames)
          try
          {
            ...
            if (Metabank.INVSTRCMP.Equals(app, SysConsts.APP_NAME_GDIDA))
            {
              ...
              {
                if (aid> NFX.DataAccess.Distributed.GDID.AUTHORITY_MAX)
                  output.Add( new MetabaseValidationMsg(MetabaseValidationMessageType.Error,
                                                    Catalog,
                                                    this,
