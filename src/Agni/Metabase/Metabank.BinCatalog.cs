              ...
              public sealed class PackageInfo
              {
                ...
                internal PackageInfo(string fullName)
                {
                  if (fullName.IsNullOrWhiteSpace())
                    throw new AgniException(StringConsts.ARGUMENT_ERROR + "PackageInfo.ctor(fullName=null|empty)");

                  var segs = fullName.Split('.');
                  if (segs.Length<4)
                    throw new AgniException(StringConsts.ARGUMENT_ERROR + "PackageInfo.ctor(!name.plat.os.ver)");
                ...
                public PackageInfo(string name, string version, string platform = ANY, string os = ANY)
                {
                  if (name.IsNullOrWhiteSpace() || version.IsNullOrWhiteSpace())
                    throw new AgniException(StringConsts.ARGUMENT_ERROR + "PackageInfo.ctor(name|ver=null|empty)");
