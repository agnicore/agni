...
namespace Agni.Contracts
{
  ...
  /// Consumers should use AgniWebMessageSink via MessageService instance to route messages
  /// </summary>
  [Glued]
  [LifeCycle(ServerInstanceMode.Singleton)]
  public interface IWebMessageSystem : IAgniService
  {
    /// <summary>
    ...
    /// </summary>
    MsgSendInfo[] SendMessage(AgniWebMessage msg);
    ...
    /// </summary>
    AgniWebMessage FetchMailboxMessage(MailboxMsgID mid);

    /// <summary>
    /// Fetches an attachment for the specified message by id and attachment index or null if not found
    /// </summary>
    AgniWebMessage.Attachment FetchMailboxMessageAttachment(MailboxMsgID mid, int attachmentIndex);
  }

  /// <summary>
  /// Contract for client of IWebMessageStore svc
  /// </summary>
  public interface IWebMessageSystemClient : IAgniServiceClient, IWebMessageSystem
  {
    CallSlot Async_SendMessage(AgniWebMessage data);
  ...
  {
    public MessageHeaders(MailboxID xid, AgniWebMessage[] headers) { Mailbox = xid; Headers = headers;}

    public readonly MailboxID Mailbox;
    public readonly AgniWebMessage[] Headers;
