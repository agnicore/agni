...
{
  /// <summary>
  /// Represents a centralized manager and client factory for obtaining IAgniServiceClient-implementing instances
  ...
  public class ServiceClientHub : ApplicationComponent
  {
    ...
    public sealed class ContractMapping : INamed
    {
      public ContractMapping(IConfigSectionNode config)
      {
        try
        {
          ...
          if (cname.IsNullOrWhiteSpace()) throw new Clients.AgniClientException(CONFIG_MAP_CLIENT_CONTRACT_ATTR + " is unspecified");
        ...
        {
          throw new Clients.AgniClientException(StringConsts.AGNI_SVC_CLIENT_MAPPING_CTOR_ERROR.Args(
                                              config.ToLaconicString(NFX.CodeAnalysis.Laconfig.LaconfigWritingOptions.Compact),
      ...
      public sealed class Data
      {
        ...
        internal Data(IConfigSectionNode baseConfig, IConfigSectionNode config, bool global)
        {
          ...
          if (cname.IsNullOrWhiteSpace()) throw new Clients.AgniClientException(CONFIG_MAP_IMPLEMENTOR_ATTR + " is unspecified");
          m_Implementor = Type.GetType(cname, true);

          if (!m_Implementor.GetInterfaces().Any(i => i == typeof(IAgniServiceClient)))
            throw new Clients.AgniClientException("Implementor {0} is not IAgniServiceClient".Args(m_Implementor.FullName));
    ...
    /// </summary>
    public static TServiceClient New<TServiceClient>(string toHost, string fromHost = null, string svcName = null) where TServiceClient : class, IAgniServiceClient
    {
    ...
    /// </summary>
    public static void TestSetupOf<TServiceClient>(string toHost, string fromHost = null, string svcName = null) where TServiceClient : class, IAgniServiceClient
    {
    ...
    public static TResult CallWithRetry<TServiceClient, TResult>(Func<TServiceClient, TResult> callBody,
                                                                 ...
                                                                 string svcName = null
                                                                ) where TServiceClient : class, IAgniServiceClient
    {
      if (callBody == null)
        throw new Clients.AgniClientException(StringConsts.ARGUMENT_ERROR + typeof(ServiceClientHub).Name + ".CallWithRetry<{0}>(callBody==null)".Args(typeof(TServiceClient).Name));

      if (hosts == null || !hosts.Any())
        throw new Clients.AgniClientException(StringConsts.ARGUMENT_ERROR + typeof(ServiceClientHub).Name + ".CallWithRetry<{0}>(hosts==null|[0])".Args(typeof(TServiceClient).Name));
          ...
          catch (Exception error)
          {
            App.Log.Write(
              new NFX.Log.Message
              {
                ...
                Source = 248,
                Text = StringConsts.AGNI_SVC_CLIENT_HUB_RETRY_CALL_HOST_ERROR.Args(typeof(TServiceClient).FullName, host),
                Exception = error
              }
            );
      ...
      throw new Clients.AgniClientException(StringConsts.AGNI_SVC_CLIENT_HUB_CALL_RETRY_FAILED_ERROR.Args(typeof(TServiceClient).Name, hosts.Count()));
    }
    ...
    public static void CallWithRetry<TServiceClient>(Action<TServiceClient> callBody,
                                                     ...
                                                     Func<TServiceClient, Exception, bool> abortFilter = null,
                                                     string fromHost = null,
                                                     string svcName = null) where TServiceClient : class, IAgniServiceClient
    {
      if (callBody == null)
        throw new Clients.AgniClientException(StringConsts.ARGUMENT_ERROR + typeof(ServiceClientHub).Name + ".CallWithRetry<{0}>(callBody==null)".Args(typeof(TServiceClient).Name));
                                                                ...
                                                                ) where TServiceClient : class, IAgniServiceClient
    {
      ...
      tcs.TrySetException(new Clients.AgniClientException(
            StringConsts.AGNI_SVC_CLIENT_HUB_CALL_RETRY_FAILED_ERROR.Args(typeof(TServiceClient).Name, hostsCount)));
    }
    ...
    /// </summary>
    public TServiceClient GetNew<TServiceClient>(string toHost, string fromHost = null, string svcName = null) where TServiceClient : class, IAgniServiceClient
    {
      Type tcontract = typeof(TServiceClient);

      if (toHost.IsNullOrWhiteSpace())
        throw new Clients.AgniClientException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".GetByContract<{0}>(host==null|empty)".Args(tcontract.Name));

      if (fromHost.IsNullOrWhiteSpace()) fromHost = AgniSystem.HostName;

      if (fromHost.IsNullOrWhiteSpace())
        throw new Clients.AgniClientException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".GetByContract<{0}>(fromHost==null|empty & AgnySystem is not avail)".Args(tcontract.Name));
    ...
    protected ContractMapping MapContractToImplementation(Type tContract)
    {
      try
      {
        ...
        if (mapped == null) throw new Clients.AgniClientException("Map['{0}'] -> <null>".Args(tContract.FullName));
        return mapped;
      }
      catch (Exception error)
      {
        throw new Clients.AgniClientException(StringConsts.AGNI_SVC_CLIENT_HUB_MAPPING_ERROR.Args(tContract.FullName, error.ToMessageWithType()), error);
      }
    }
    ...
    protected static readonly Type[] CTOR_SIG_GLUE_ENDPOINT = new Type[] { typeof(NFX.Glue.Node), typeof(NFX.Glue.Binding) };
  ...
  namespace Instrumentation
  {
    ...
    [Serializable]
    public abstract class ServiceClientHubEvent : NFX.Instrumentation.Event, NFX.Instrumentation.INetInstrument
    {
      protected ServiceClientHubEvent(string src) : base(src) { }
    }

    [Serializable]
    public abstract class ServiceClientHubErrorEvent : ServiceClientHubEvent, NFX.Instrumentation.IErrorInstrument
    {
    ...
    public class ServiceClientHubRetriableCallError : ServiceClientHubErrorEvent
    {
      ...
      protected override NFX.Instrumentation.Datum MakeAggregateInstance()
      {
