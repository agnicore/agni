...
namespace Agni.Log
{
  ...
  /// </summary>
  public sealed class AgniZoneDestination : Destination
  {
    ...
    public AgniZoneDestination() : base() { }
    public AgniZoneDestination(string name) : base(name) { }
    ...
    protected override void DoPulse()
    {
      ...
      var flushEvery = FLUSH_INTERVAL_MS + NFX.ExternalRandomGenerator.Instance.NextScaledRandomInteger(0, 5000);
    ...
    private void flush()
    {
      ...
      try
      {
        var myHost = AgniSystem.HostName;
        ...
        using (var client = Contracts.ServiceClientHub.New<Contracts.IZoneLogReceiverClient>( zgHost ))
        {
          ...
          var expect = client.SendLog(myHost, AgniSystem.MetabaseApplicationName, m_Buf.ToArray());
