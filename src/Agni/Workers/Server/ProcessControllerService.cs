  ...
  public class ProcessControllerService : ServiceWithInstrumentationBase<object>, Contracts.IProcessController, IProcessHost
  {
    ...
    private void spawnCore(ProcessFrame frame, object tx)
    {
      ...
      try
      {
        ...
        var processExisting = existing.Materialize(AgniSystem.ProcessManager.ProcessTypeResolver);
    ...
    private void put(ProcessFrame frame, object transaction = null)
    {
      try
      {
        frame.Descriptor = new ProcessDescriptor(frame.Descriptor, ProcessStatus.Started, "Started", App.TimeSource.UTCNow, "@{0}@{1}".Args(App.Name, AgniSystem.HostName));
