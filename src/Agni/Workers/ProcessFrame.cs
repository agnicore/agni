  ...
  public struct ProcessFrame
  {
    ...
    public ProcessFrame(Process process, int? serializer = SERIALIZER_DEFAULT)
    {
      ...
      if (serializer.HasValue)
      {
        ...
        try
        {
          var cdoc = RowConverter.DefaultInstance.RowToBSONDocument(process, null);
    ...
    public Process Materialize(IGuidTypeResolver resolver)
    {
      ...
      var result = (Process)NFX.Serialization.SerializationUtils.MakeNewObjectInstance( type );
      ...
      try
      {
        ...
        RowConverter.DefaultInstance.BSONDocumentToRow(docContent, result, null);
      }
      catch(Exception error)
      {
